 ...
 *  edges has coordinates (x, y).
 *  For cells, x and y are positive and odd.
 *  For intersections, x and y are even.
 *  For horizontal edges, x is odd and y is even.
 *  For vertical edges, x is even and y is odd.
 *  On a board with w columns and h rows of cells,
 *  (0, 0) indicates the bottom left
...
class Model {
    ...
    /** Declaring centers, boundaries, and marks as Model class variables. */
    private int[][] centers, boundaries, marks;

    /** Declaring cols and rows as Model class variables. */
    ...
    /** Initializes an empty puzzle board of size COLUMN x ROW, with a boundary
    ...
    void copy(Model model) {
        if (model == this) {
            ...
            init(model.cols, model.rows);
            for (int i = 0; i < xlim(); i += 1) {
                for (int n = 0; n < ylim(); n += 1) {
                    this.centers[i][n] = model.centers[i][n];
                    this.boundaries[i][n] = model.boundaries[i][n];
                    this.marks[i][n] = model.marks[i][n];
    ...
    /** Sets the puzzle board size to COLUMN x ROW, and clears it. */
    void init(int column, int row) {
        this.cols = column;
        this.rows = row;
        this.centers = new int[xlim()][ylim()];
    ...
    boolean isBoundary(int x, int y) {
        boolean q = x == 0 || x == xlim() - 1 || y == 0 || y == ylim() - 1;
        if (boundaries[x][y] == 1) {
            return !q;
        }
        return q;
        ...
        for (Place cell : region) {
            ...
            for (int i = 0; i < 4; i += 1) {
                ...
                if (region.contains(nextCell)
                        && !region.contains(opposing(center, nextCell))) {
                    return false;
                } else if (region.contains(nextCell) && isBoundary(boundary)) {
                    return false;
                } else if (isCenter(boundary) && (center != boundary)) {
                    return false;
                }
            }
            for (int i = 0; i < 4; i += 1) {
    ...
    HashSet<Place> findGalaxy(Place center) {
        ...
        if (isCell(center)) {
            ...
            for (int i = 0; i < 4; i += 1) {
                ...
                galaxy.add(center.move(dx, dy));

            }
        }
        HashSet<Place> dup = new HashSet<>();
        for (Place spot: galaxy) {
            accreteRegion(spot, dup);
        }
        galaxy = dup;
    ...
    Set<Place> maxUnmarkedRegion(Place center) {
        ...
        List<Place> spot = new ArrayList();
        while (spot.size() != region.size()) {
            spot = new ArrayList(region);
            region.addAll(unmarkedSymAdjacent(center, spot));
            markAll(region, 1);
        }
        markAll(region, 0);
        return region;
    ...
    Place opposing(Place p0, Place p) {
        if (isCell(p0.x - (p.x - p0.x), p0.y - (p.y - p0.y))) {
            Place tmp = pl(p0.x - (p.x - p0.x), p0.y - (p.y - p0.y));
            if (isCell(tmp)) {
                return tmp;
            } else {
                return null;
    ...
    List<Place> centers() {
        ArrayList<Place> cntrs = new ArrayList<>();
        for (int i = 0; i < xlim(); i += 1) {
            for (int n = 0; n < ylim(); n += 1) {
                if (isCenter(i, n)) {
                    cntrs.add(pl(i, n));
        ...
        return Collections.unmodifiableList(cntrs);
    }
